...
class QuantPatchEmbedding(nn.Module):
    ...
    def forward(self, x):
        x = fixed_point_rep(x, quant_bits=8)
        b, _, _, _ = x.shape
        x = self.projection(x) / self.wscale #8 bit fixed point activation
        cls_token = repeat(self.cls_token, '() s d -> b s d', b=b)
        dist_token = repeat(self.distillation_token, '() s d -> b s d', b=b)

        x = torch.cat([cls_token, dist_token, x], dim=1)
        return fixed_point_rep(x + self.pos_embed, quant_bits=8) #8 bit fixed point activation

    def quant_layer(self, quant_bits=8):
        self.wscale = get_scale_factor(self.projection[1].weight, quant_bits=quant_bits, round_to_power2=True)
        self.bscale = get_scale_factor(self.projection[1].bias, quant_bits=quant_bits, round_to_power2=True)
        self.bias_dequant = self.wscale / self.bscale

        print(torch.min(self.projection[1].weight), torch.max(self.projection[1].weight))
        print(torch.min(self.projection[1].bias), torch.max(self.projection[1].bias))
...
class QuantMultiHeadAttention(nn.Module):
    ...
    def attention(self, q, k, v):
        if self.quant:
            scores = fixed_point_rep(torch.matmul(q, k.transpose(-2, -1)) / math.sqrt(self.d_k) / self.q_proj_scale / self.k_proj_scale, quant_bits=8) #back to 16 bit fixed representation
            # print('qk score:', torch.min(scores), torch.max(scores))
            scores = nn.functional.softmax(scores, dim=-1)
            scores = self.dropout(scores)
            output = self.scores_scale * torch.matmul(scores, v) / self.v_proj_scale #8 bit int quantized
        else:
    ...
    def forward(self, q, k, v):
        ...
        if self.quant:
            q = (self.inp_scale * q).int().float() #8 bit int quantized
            k = (self.inp_scale * k).int().float()
            v = (self.inp_scale * v).int().float()

            # print("q 8b int: ", torch.min(q), torch.max(q))

            q = (self.q_proj_scale * self.q_linear(q).view(b, -1, self.heads, self.d_k).transpose(1, 2) / self.q_wscale / self.inp_scale).int().float() #want (batch, heads, seq_len, d_k)
            k = (self.k_proj_scale * self.k_linear(k).view(b, -1, self.heads, self.d_k).transpose(1, 2) / self.k_wscale / self.inp_scale).int().float() #8 bit int quantized
            v = (self.v_proj_scale * self.v_linear(v).view(b, -1, self.heads, self.d_k).transpose(1, 2) / self.v_wscale / self.inp_scale).int().float()

            # print("qproj:", torch.min(q), torch.max(q))
            # print("kproj:", torch.min(k), torch.max(k))
            # print("vproj:", torch.min(v), torch.max(v))

            scores = self.attention(q, k, v).transpose(1, 2).reshape(b, -1, self.d_model).int().float()  #8 bit int quantized

            # print("scores:", torch.min(scores), torch.max(scores))

            output = fixed_point_rep(self.out(scores) / self.out_wscale / self.scores_scale, quant_bits=8) #back to 8-bit fixed representation
    ...
    def quant_layer(self, quant_bits=8):
        ...
        self.q_wscale = get_scale_factor(self.q_linear.weight, quant_bits=quant_bits, round_to_power2=True)
        self.q_bscale = get_scale_factor(self.q_linear.bias, quant_bits=quant_bits, round_to_power2=True)
        self.qbias_dequant = self.inp_scale * self.q_wscale / self.q_bscale

        self.k_wscale = get_scale_factor(self.k_linear.weight, quant_bits=quant_bits, round_to_power2=True)
        self.k_bscale = get_scale_factor(self.k_linear.bias, quant_bits=quant_bits, round_to_power2=True)
        self.kbias_dequant = self.inp_scale * self.k_wscale / self.k_bscale

        self.v_wscale = get_scale_factor(self.v_linear.weight, quant_bits=quant_bits, round_to_power2=True)
        self.v_bscale = get_scale_factor(self.v_linear.bias, quant_bits=quant_bits, round_to_power2=True)
        self.vbias_dequant = self.inp_scale * self.v_wscale / self.v_bscale

        self.out_wscale = get_scale_factor(self.out.weight, quant_bits=quant_bits, round_to_power2=True)
        self.out_bscale = get_scale_factor(self.out.bias, quant_bits=quant_bits, round_to_power2=True)
...
class QuantEncoderBlock(nn.Module):
    ...
    def quant_layer(self, quant_bits=8):
        ...
        self.inter_wscale = get_scale_factor(self.ff_linear.weight, quant_bits=quant_bits, round_to_power2=True)
        self.inter_bscale = get_scale_factor(self.ff_linear.bias, quant_bits=quant_bits, round_to_power2=True)
        self.inter_bias_dequant = self.inter_wscale / self.inter_bscale

        self.out_wscale = get_scale_factor(self.out_linear.weight, quant_bits=quant_bits, round_to_power2=True)
        self.out_bscale = get_scale_factor(self.out_linear.bias, quant_bits=quant_bits, round_to_power2=True)
...
class QuantViT(LightningModule):
    ...
    def quant_layer(self, quant_bits):
        ...
        self.class_wscale = get_scale_factor(self.classification.weight, quant_bits=quant_bits, round_to_power2=True)
        self.class_bscale = get_scale_factor(self.classification.bias, quant_bits=quant_bits, round_to_power2=True)
...
def fixed_point_rep(tensor, quant_bits=8):
    ...
    f = (1<<quant_bits)
    return torch.round(tensor*f) * (1.0/f)
...
if __name__ == "__main__":
    ...
    testset = torchvision.datasets.CIFAR10(root='./data', train=False,
                                        download=True, transform=transform)
    testset_1 = torch.utils.data.Subset(testset, list(range(10)))
