...
def copy_model(relu_model, deit_model):
    ...
    relu_model.embedder.projection[1].weight = nn.Parameter(torch.permute(deit_model.model.deit.embeddings.patch_embeddings.projection.weight, [0, 3, 2, 1]).contiguous().view(-1, config.hidden_size))
    relu_model.embedder.projection[1].bias = deit_model.model.deit.embeddings.patch_embeddings.projection.bias
    ...
    #copy Encoder layers
    for i in range(deit_model.model.deit.config.num_hidden_layers):
...
deit_model.load_state_dict(torch.load('/home/edith/Documents/projects/ViT/saved_model/final_model'))
config = deit_model.model.deit.config
...
copy_model(quant_model, deit_model)
...
torch.save(quant_model.state_dict(), '/home/edith/Documents/projects/ViT/saved_model/test_dense_relu_deit')
